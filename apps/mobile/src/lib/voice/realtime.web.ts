  ...
  return {
    async start(secret: string) {
      ...
      try {
        ...
        audio.autoplay = true;
        peer.ontrack = (event) => { if (audio) audio.srcObject = event.streams[0]; };
        ...
        callbacks.onError(error instanceof Error ? error.message : 'Unable to start the engineer call.');
    ...
    sendText(text) {
      ...
      stream?.getTracks().forEach((track) => track.stop()); channel?.close(); peer?.close();
      audio?.remove(); callbacks.onStatus('ended');
    },
  };
}
